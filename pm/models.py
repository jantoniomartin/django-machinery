...
class Project(models.Model):
    ...
    hide = models.BooleanField(_("hide from dashboard"), default=False)
...
class Machine(models.Model):
    ...
    description = models.CharField(_("description"), max_length=255)
    created_on = models.DateField(_("created on"), auto_now_add=True, null=True)
    finished_on = models.DateField(_("finished on"), null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.shipped_on and not self.finished_on:
            self.finished_on = self.shipped_on
...
def decrease_stock(sender, instance, created, raw, using, **kwargs):
    if created:
        instance.article.update_stock (-1 * instance.quantity)
